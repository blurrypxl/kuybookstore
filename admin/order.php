									...
									<table id="dataTable3" class="display" style="width:100%">
										...
										<tbody>
											...
											while ($p = mysqli_fetch_array($brgs)) {
												if ($p['hargaafter'] == 0) {
													$total = $p['qty'] * $p['hargabefore'];

													$result = mysqli_query($conn, "SELECT SUM(d.qty*p.hargabefore) AS count FROM detailorder d, produk p where orderid='$orderids' and d.idproduk=p.idproduk order by d.idproduk ASC");
												}
												if (!$p['hargaafter'] == 0) {
											...
											<tr>
												...
												<th>Rp
													...
													if ($hargaProduk['hargaafter'] == 0) {
														$result1 = mysqli_query($conn, "SELECT SUM(d.qty*p.hargabefore) AS count FROM detailorder d, produk p where orderid='$orderids' and d.idproduk=p.idproduk order by d.idproduk ASC");
													}
													if (!$hargaProduk['hargaafter'] == 0) {
